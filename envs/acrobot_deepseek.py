...
class AcrobotEnv(core.Env):
    ...
    def compute_reward(self, states: np.ndarray, actions:np.ndarray, terminated:bool) -> float:
        """
        Computes the reward for the swinging chain task.
    
        Args:
            states (np.ndarray): The state of the system.
            actions (np.ndarray): The actions taken by the agent.
        ...
        # Reward for height above the initial height
        height_change = states[3]  # Sine of theta2 (height above initial)
        height_reward = height_change * 10.0  # Scale the reward
    
        # Reward for applying torque (encourage movement)
        torque_reward = abs(actions[0]) * 2.0 # scale the reward
    
        # Penalize large torques
        torque_penalty = 0.1 * np.square(actions[0])
        reward = height_reward + torque_reward - torque_penalty
