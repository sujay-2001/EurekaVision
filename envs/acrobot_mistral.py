...
class AcrobotEnv(core.Env):
    ...
    def compute_reward(self, states, actions, terminated):
        # Accessing states and actions using their respective numbers
        theta1_cos = states[0]  # Cosine of `theta1`
        theta1_sin = states[1]  # Sine of `theta1`
        theta2_cos = states[2]  # Cosine of `theta2`
        theta2_sin = states[3]  # Sine of `theta2`
        d_theta1_dt = states[4]  # Angular velocity of `theta1`
        d_theta2_dt = states[5]  # Angular velocity of `theta2`
    
        # Accessing action using its number (0, 1 or 2)
        torque = actions[int(np.round(actions))]  # Torque applied to the actuated joint
    
        # Reward components for each aspect you want to encourage (e.g., height, velocity, stability)
        height_reward = np.exp(-((theta2_cos)**2)) if not terminated else 0  # Higher rewards for higher heights
        velocity_reward = -np.abs(d_theta1_dt)  # Lower rewards for high absolute velocities to encourage smooth movements
        stability_reward = np.exp(-((theta1_cos - theta2_cos)**2)) if not terminated else 0  # Higher rewards for stable configurations (close to a straight line)
    
        # Normalizing the reward by summing the individual components
        total_reward = height_reward + velocity_reward + stability_reward
